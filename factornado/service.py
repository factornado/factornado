...
class AbstractService :
    ...
    def get_logger(self):
        if self.config['log.level'] < 10 :
            ...
            logging.basicConfig(
                level=self.config['log.level'],
                format='%(asctime)s %(levelname)s %(message)s',
                filename=self.config['log.folder'] + 'arabica-' + self.service_name + '.log',
                filemode='a')
    ...
    def get_mongo_collections(self):
        ...
        for coll, coll_config in mongo_collection_config.items():
            if not coll in self.mongo_databases[coll_config['database']].collection_names():
                self.logger.error('MONGO - could not find collection ' + coll)
                raise Exception('could not find collection ' + coll)

            mongo_collections[coll] = self.mongo_databases[coll_config['database']][coll]
